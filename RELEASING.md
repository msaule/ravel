...
After the first successful build, add the install instructions and
badges from the R-universe dashboard to `README.md`.
...
- Revalidate live provider support before release if auth behavior
  changed.
- Keep provider claims conservative and official.
- Do not advertise consumer-login support for providers that only expose
  API-key auth.
